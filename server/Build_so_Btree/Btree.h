...
public:
    ...
    void remove(K k);
    V search(K k);
    void traverse(std::stringstream& ss);
...
private:
    ...
    void traverse(BNT* node, int d, std::stringstream& ss);
...
KV
void BT::traverse(std::stringstream& ss) {
    traverse(root, 1, ss);
}

KV
void BT::traverse(BNT* node, int d, std::stringstream& ss) {
    int i;
    for (i = 0; i < node->n; i++) {
        if (!node->isLeaf)
            traverse(node->children[i], d + 1, ss);
